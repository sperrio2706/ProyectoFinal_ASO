...
def pam_sm_authenticate(pamh, flags, argv):
    ...
    try:
        usuarios = ""
        usuarios = subprocess.check_output(["/bin/getent", "passwd"])
        usuarios = usuarios.decode("utf-8")
        ...
        if coincidencias:
            idmap_inicio = int(coincidencias.group(1))
            idmap_fin = int(coincidencias.group(2))
        ...
        # Si el usuario actual es igual al usuario y el uid está entre lo que lea
            if usuario_actual == usuario and idmap_inicio <= uid and uid <= idmap_fin:
